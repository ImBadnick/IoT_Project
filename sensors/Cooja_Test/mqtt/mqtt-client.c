...
PROCESS_THREAD(energy_control_process, ev, data)
{
    ...
    while(1) {
        ...
        if(state == STATE_SUBSCRIBED){
            ...
            snprintf(app_buffer, APP_BUFFER_SIZE, "{\"node\": %d, \"energy_consumption\": %d, \"sound_alarm_on\": %d, \"timestamp\": %lu}", 
                    node_id, energy_consumption, (int) sound_alarm_on, clock_seconds());
